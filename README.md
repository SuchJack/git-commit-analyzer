...
**Git Commit Analyzer** 是一款基于 IntelliJ IDEA 的插件，用于**使用大模型（LLM）分析 Git 提交**。在 IDE 的 Git Log 中选中任意一条 commit，右键选择「AI 分析该提交」，即可获得该次提交的**变更摘要、风险评估与合并建议**，并可在结果弹窗中一键 **Cherry-Pick** 该提交。

### 主要用途

- **代码评审**：快速理解某次提交改了什么、有没有风险，辅助 Code Review。
- **合并前决策**：在合并分支或 Cherry-Pick 前，先看 AI 的分析与建议，再决定是否合入。
- **历史追溯**：对任意历史提交做「事后分析」，便于复盘与文档化。

### 核心能力

- 在 **Git Log** 中右键单条 commit 即可触发分析，无需离开 IDE。
- 支持任意 **OpenAI 兼容 API**（OpenAI、DeepSeek、通义、本地模型等），在设置中配置 API 地址、Key 与模型即可。
- 分析结果包含：变更摘要、潜在风险、是否建议合并等，并可查看原始 Diff。
- 分析完成后可直接在弹窗中执行 **Cherry-Pick**，无需切到终端或 Git 工具窗口。
